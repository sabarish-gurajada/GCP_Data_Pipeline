...
echo "Enter service account created for SQL task"
read SERVICE_ACCOUNT
echo "Enter Json file along with its path for authetication"
read SERVICE_ACCOUNT_PATH
echo "Please wait while authenticating"

gcloud auth activate-service-account ${SERVICE_ACCOUNT} --key-file=${SERVICE_ACCOUNT_PATH}
...
echo "---------------------------"
read GSUTIL_FOLDER
gsutil -o "GSUtil:encryption_key=${KEY_GEN}" cp -r ${GSUTIL_FOLDER} ${HOME}/ejournal/
...
mysql --host=$MYSQLIP --user=root --password --verbose < ${HOME}/ejournal/GCP_Ejournal/create_table.sql



#  Import CSV file into cloud SQL

mysqlimport --local --host=$MYSQLIP --user=root --password --ignore-lines=1 --fields-terminated-by=',' Ejournal ${HOME}/ejournal/GCP_Ejournal/Ejournal_live_data.csv-*
